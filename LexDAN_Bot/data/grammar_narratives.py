...
NARRATIVE_INTROS: dict[str, str] = {
    ...
    "simple_phrases": (
        ...
        "Hello! — более формально (учитель, незнакомые). "
        "Hi! — с друзьями, неформально. "
    ...
    "plurals": (
        ...
        "После -s/-ss/-x/-ch/-sh часто -es: box → boxes, watch → watches.\n"
        "Согласная + y → ies: city → cities (но day → days).\n\n"
        "<b>Главные исключения</b> (выучи списком):\n"
        "• man → men, woman → women\n"
        "• child → children\n"
        "• foot → feet, tooth → teeth\n"
        "• mouse → mice\n"
        "• person → people\n"
        "• sheep → sheep, fish → fish (часто без изменения)\n"
        "• knife → knives, leaf → leaves\n\n"
        "Two books — <i>Две книги</i>, Three children — <i>Трое детей</i>. "
        "Запоминай неправильные формы — потом они идут сами 😄"
    ),
    "there_is_are": (
        "🦜 <b>Рико:</b> Знакомься с конструкцией <b>There is / There are</b> — "
        "по-русски это «есть / имеется».\n\n"
        "<b>Лайфхак:</b> когда в начале There is/are — удобно <b>переводить предложение с конца</b>.\n"
        "Пример: There is a phone in the bag → сначала «в сумке», потом «есть телефон».\n"
        "Если сумка конкретная (эта, о которой говорим) — <b>the bag</b>. "
        "Если любая/неопределённая — a bag.\n\n"
        "There is a cat on the sofa — <i>На диване есть кот</i> (один). "
        "There are two books — <i>Есть две книги</i>. "
        "Вопрос: Is there a shop near here?\n\n"
        "Не путай с It is! There is a problem — <i>Есть проблема</i>, "
        "а It is a problem — <i>Это проблема</i>. "
        "Единственное → is, множественное → are ✅"
    ),
    "can_ability": (
