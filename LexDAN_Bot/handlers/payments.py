...
async def _start_checkout(c: CallbackQuery, plan: str) -> None:
    ...
    locked_full = has_lifetime_full_price(user) and plan == PLAN_FULL
    if plan == PLAN_UPGRADE:
        ...
        description = f"LexDAN: {title} на 30 дней (персональная цена)"
        pay_blurb = (
            f"💳 <b>{title} — {price}₽/мес</b>\n"
            "🏷 Это твоя <b>персональная цена навсегда</b> "
            "(даже если каталог подорожает).\n\n"
            f"{discount_blurb(user)}"
            "После оплаты подписка включится автоматически "
            "(обычно сразу, иногда до пары минут).\n"
            "Карта сохранится для <b>автопродления</b> по этой же цене — "
