...
def hub_intro(user: dict) -> str:
    ...
    free = user_plan(user) != "full"
    if ui_preview_only(user=user):
        cap = "2" if free else "4"
        hint = " (слово + фраза)" if free else ""
        return (
            f"✨ Маленький ритуал на сегодня: искры от Рико{hint} 🔥\n\n"
            f"📊 Открыто сегодня: <b>{n}</b>/{cap} · обновление в <b>00:00 МСК</b> 🕛"
        )

    lines = [
        "🔥 <b>Огонь дня</b>\n",
        "Маленький ритуал на сегодня: искры от Рико — "
        "не банальщина из учебника, а штуки, от которых язык оживает ✨\n",
    ]
    if free:
        lines.append(
            f"Бесплатно сегодня: <b>слово</b> и <b>фраза</b> · открыто <b>{n}</b>/2 · "
            "сброс в <b>00:00 МСК</b>\n"
            "🎙 Голос и 💡 факт — с безлимитом.\n"
        )
    else:
        lines.append(f"Открыто сегодня: <b>{n}</b>/4 · обновление в <b>00:00 МСК</b>\n")
    lines.extend(
        [
            ...
            "Каждый новый день — <b>новый</b> контент 🔄",
        ]
    )
    return "\n".join(lines)
