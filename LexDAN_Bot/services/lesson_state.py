...
# Короткий экран «Задания по теме» (онбординг + весь Grammar)
GRAMMAR_TASKS_OVERVIEW_HTML = (
    "📝 <b>Задания по теме</b>\n\n"
    "Всего 8 заданий. Сложность растёт от 1 к 8! 📈✨\n\n"
    ...
    "Пройдены все 8 заданий → появляется галочка на теме ✅. "
    "Пройдены все темы → откроется тест по Grammar. Удачи! 🍀🚀💚"
)
...
def clear_lesson(user_id: str) -> None:
    ...
    def mut(u):
        ...
        # сброс незавершённой сессии Listening/Reading (галочки тем сохраняются)
        from services.listening_state import ensure_listening
        from services.reading_state import ensure_reading
